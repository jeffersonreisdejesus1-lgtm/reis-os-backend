...
def decide(receipt: IncrementReceipt, requirements: DecisionRequirements = DecisionRequirements()) -> Decision:
    receipt.validate()
    ...
    missing_tests = set(requirements.required_test_commands) - set(receipt.test_commands)
    if missing_tests:
    ...
    if requirements.runtime_required and receipt.runtime_result.status is not ExecutionStatus.PASS:
        return Decision(Decision.REPAIR if receipt.runtime_result.status is ExecutionStatus.FAIL else Decision.HOLD, "runtime_requirement_unmet")
