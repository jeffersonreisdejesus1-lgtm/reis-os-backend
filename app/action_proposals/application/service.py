...
async def _get_action_proposal_for_update(
    session: AsyncSession, *, proposal_id: UUID
) -> ActionProposalModel:
    proposal = await session.scalar(
...
async def approve_action_proposal(
    ...
    proposal = await _get_action_proposal_for_update(
...
async def execute_action_proposal(
    ...
    if proposal.execution_status is not None:
        raise AppError(
    ...
    proposal.execution_status = ActionExecutionStatus.SIMULATED_SUCCESS
