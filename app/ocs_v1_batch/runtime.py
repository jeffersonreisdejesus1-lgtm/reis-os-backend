...
class OCSV1Runtime:
    ...
    def __init__(self, *, profile: CandidateProfile, binding_refs: dict[str, str], derivation_ref: str) -> None:
        profile.validate()
    ...
    def _validate(self, request: GovernanceRequest) -> tuple[str | None, bool]:
        ...
        if p.ocs_id == "DEDALA":
            ...
            if not request.object_ref or not request.exact_head:
                return "HOLD_EXACT_IMPLEMENTATION_OBJECT_HEAD_REQUIRED", True
            if not request.evidence_ref:
                return "HOLD_MISSING_TEST_OR_VERIFICATION_EVIDENCE", True

        elif p.ocs_id == "SOFIA":
            if not request.object_ref or not request.exact_head:
                return "HOLD_EXACT_REPOSITORY_OBJECT_HEAD_REQUIRED", True
            ...
            if request.promotion_requested:
                return "REGISTRY_WRITE_NOT_CANONICAL_PROMOTION", False
