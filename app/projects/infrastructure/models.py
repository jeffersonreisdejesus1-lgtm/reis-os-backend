...
class ProjectModel(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    __table_args__ = (
        ...
        Index("ix_projects_org_workspace_status", "organization_id", "workspace_id", "status"),
    )
    ...
    status: Mapped[ProjectStatus] = mapped_column(Enum(ProjectStatus, native_enum=False, length=20, values_callable=lambda e: [i.value for i in e]), default=ProjectStatus.DRAFT)
