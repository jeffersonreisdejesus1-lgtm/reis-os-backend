...
def lease(*, generation: int = 1, lease_id: str = "lease:durable:1") -> GovernorLease:
...
def test_restart_restores_state_receipts_leases_and_idempotency(tmp_path: Path) -> None:
    path = tmp_path / "r7.sqlite3"
    first = open_runtime(path)
    first.register_governor(contract())
    first.issue_lease(lease())
...
def test_post_commit_crash_is_restart_recoverable_without_duplicate(tmp_path: Path) -> None:
...
def test_recovery_fencing_survives_restart(tmp_path: Path) -> None:
    ...
    first.issue_lease(lease(generation=2, lease_id="lease:durable:2"))
    ...
    assert restarted.state_version == 2
