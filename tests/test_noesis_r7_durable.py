...
from app.noesis_r7 import (
    GovernorContract, GovernorFunction, GovernorLease, GovernanceCommand,
    GovernanceTask, R1R6IntegrationContract, R7DurableRuntime, R7InvariantError,
)
...
def contract():
    return GovernorContract("governor:durable-state", GovernorFunction.STATE,
        ("phase", "findings"), ("phase", "findings"), ("SET_STATE",), AUTH)


def lease(*, generation=1, lease_id="lease:durable:1"):
    return GovernorLease(lease_id, "governor:durable-state", MISSION, AUTH, SOURCE,
        ("state:noesis:r7",), generation, 100.0, 100.0, 500.0, 20)
...
def command(*, command_id="cmd:durable:1", idempotency_key="idem:durable:1",
            ...
            write_set=None):
    return GovernanceCommand(command_id, MISSION, "governor:durable-state", generation,
        lease_id, AUTH, "SET_STATE", write_set or {"phase": "IMPLEMENTATION"},
        idempotency_key, expected_state_version, ("state:noesis:r7",))


def open_runtime(path: Path):
    return R7DurableRuntime(mission_id=MISSION, integration=R1R6IntegrationContract.canonical(),
        database_path=path, initial_state={"phase": "INTAKE"})


def admit(rt, cmd, *, priority=1):
    rt.schedule(GovernanceTask(task_id=cmd.command_id, mission_id=MISSION,
...
def test_restart_restores_state_receipts_leases_idempotency_and_scheduler(tmp_path):
    ...
    pending = command(command_id="cmd:pending", idempotency_key="idem:pending", expected_state_version=1,
...
def test_recovery_fencing_survives_restart_and_version_is_monotonic(tmp_path):
    ...
    execute(first, command(command_id="cmd:2", idempotency_key="idem:2", expected_state_version=1,
        write_set={"findings": ["f1"]}), now=112.0)
    ...
    stale = command(command_id="cmd:stale", idempotency_key="idem:stale", expected_state_version=restarted.state_version)
...
def test_snapshot_tamper_fails_closed_on_restart(tmp_path):
    ...
    with sqlite3.connect(path) as connection:
        connection.execute("UPDATE r7_runtime_snapshot_journal SET snapshot_json=? WHERE seq=(SELECT MAX(seq) FROM r7_runtime_snapshot_journal WHERE mission_id=?)",
...
def test_stale_writer_is_fenced_and_reconciled(tmp_path):
    ...
    with pytest.raises(R7InvariantError, match="R7_DURABLE_STALE_WRITER"):
        second.schedule(GovernanceTask("cmd:second", MISSION, "governor:durable-state", "SET_STATE", 1,
            second._task_seq + 1, "cmd:second"))
