...
def contract_at(
    gate: GicaGate, state: GicaProgramState = GicaProgramState.ACTIVE, history=()
) -> GicaProgramContract:
    return GicaProgramContract(PROGRAM, gate, HEAD, state=state, gate_history=history)


def advance(
    current: GicaProgramContract,
    target: GicaGate,
...
def attacker_authority(gate: GicaGate, target: GicaGate):
    value = AuthorityEvidence(
        PROGRAM,
        f"transition:{gate.name}->{target.name}",
        HEAD,
        ATTACKER,
        ATTACKER,
...
def legitimate_ga12() -> GicaProgramContract:
    return advance(
        contract_at(GicaGate.GA11),
        GicaGate.GA12,
        evidence(GicaGate.GA11, assurance_pass=True),
    )
...
def test_ga2_to_ga3_denied_without_ga2_specific_assurance_pass() -> None:
    with pytest.raises(
        ProgramTransitionError, match="independent_assurance_pass_required"
    ):
        advance(
            contract_at(GicaGate.GA2),
            GicaGate.GA3,
            evidence(GicaGate.GA2, assurance_pass=False),
        )


def test_absent_authority_denied() -> None:
    with pytest.raises(ProgramTransitionError, match="valid_authority_required"):
        contract_at(GicaGate.GA3).transition_to(
            GicaGate.GA4, authority=None, gate_evidence=evidence(GicaGate.GA3), now=NOW
        )
...
def test_stale_authority_denied() -> None:
    ...
    with pytest.raises(
        ProgramTransitionError, match="stale_or_invalid_evidence_denied"
    ):
...
def test_wrong_object_authority_denied() -> None:
    ...
    with pytest.raises(
        ProgramTransitionError, match="authority_wrong_object_version_denied"
    ):
        advance(contract_at(GicaGate.GA3), GicaGate.GA4, authority_evidence=wrong)
...
def test_evidence_absent_denied() -> None:
    with pytest.raises(ProgramTransitionError, match="gate_evidence_required"):
        contract_at(GicaGate.GA3).transition_to(
            GicaGate.GA4,
            authority=authority(GicaGate.GA3, GicaGate.GA4),
            gate_evidence=None,
            now=NOW,
        )
...
def test_unbound_evidence_denied() -> None:
    with pytest.raises(ProgramTransitionError, match="evidence_provenance_required"):
        advance(
            contract_at(GicaGate.GA3),
            GicaGate.GA4,
            gate_evidence=evidence(GicaGate.GA3, provenance=""),
        )


def test_wrong_head_evidence_denied() -> None:
    with pytest.raises(
        ProgramTransitionError, match="evidence_wrong_object_version_denied"
    ):
        advance(
            contract_at(GicaGate.GA3),
            GicaGate.GA4,
            gate_evidence=evidence(GicaGate.GA3, object_version="wrong-head"),
        )
...
def test_wrong_gate_evidence_denied() -> None:
    with pytest.raises(ProgramTransitionError, match="evidence_wrong_gate_denied"):
        advance(
            contract_at(GicaGate.GA3),
            GicaGate.GA4,
            gate_evidence=evidence(GicaGate.GA2),
        )


def test_wrong_criteria_version_evidence_denied() -> None:
    with pytest.raises(
        ProgramTransitionError, match="evidence_wrong_criteria_version_denied"
    ):
        advance(
            contract_at(GicaGate.GA3),
            GicaGate.GA4,
            gate_evidence=evidence(GicaGate.GA3, criteria_version="wrong"),
        )
...
def test_empty_or_invalid_evidence_lineage_denied() -> None:
    with pytest.raises(ProgramTransitionError, match="evidence_lineage_required"):
        advance(
            contract_at(GicaGate.GA3),
            GicaGate.GA4,
            gate_evidence=evidence(GicaGate.GA3, lineage=()),
        )
...
def test_founder_authorization_outside_ga12_denied() -> None:
    with pytest.raises(
        ProgramTransitionError, match="founder_promotion_outside_ga12_denied"
    ):
        contract_at(GicaGate.GA11).founder_promote(authorization=founder(), now=NOW)


def test_directly_constructed_ga12_ready_state_cannot_complete_without_legitimate_transition_history() -> (  # noqa: E501
    None
):
    with pytest.raises(
        ProgramTransitionError, match="legitimate_ga12_transition_required"
    ):
        contract_at(GicaGate.GA12, GicaProgramState.READY_FOR_FOUNDER).founder_promote(
            authorization=founder(), now=NOW
        )
...
def test_attacker_created_authority_root_cannot_be_passed_to_transaction() -> None:
    attacker_root = _InstitutionalTrustRoot(
        ATTACKER_KEY, ATTACKER_KEY, ATTACKER_KEY, "attacker://root"
    )
    with pytest.raises(TypeError):
        contract_at(GicaGate.GA2).transition_to(
            GicaGate.GA3,
...
def test_attacker_created_authority_key_denied() -> None:
    malicious = sign(
        replace(authority(GicaGate.GA3, GicaGate.GA4), signature=""), ATTACKER_KEY
    )
    with pytest.raises(ProgramTransitionError, match="authority_forged_denied"):
...
def test_attacker_selected_founder_issuer_denied() -> None:
    target = legitimate_ga12()
    with pytest.raises(
        ProgramTransitionError, match="founder_reserved_authority_required"
    ):
...
def test_attacker_selected_founder_key_denied() -> None:
    ...
    with pytest.raises(
        ProgramTransitionError, match="founder_authorization_forged_denied"
    ):
...
def test_entirely_self_rooted_founder_universe_denied() -> None:
    target = contract_at(
        GicaGate.GA12, GicaProgramState.READY_FOR_FOUNDER, history=(GicaGate.GA11,)
    )
    with pytest.raises(ProgramTransitionError):
        target.founder_promote(authorization=attacker_founder(), now=NOW)
...
def test_externally_anchored_independent_ga2_assurance_permits_ga2_to_ga3() -> None:
    assert (
        advance(
            contract_at(GicaGate.GA2),
            GicaGate.GA3,
            evidence(GicaGate.GA2, assurance_pass=True),
        ).gate
        is GicaGate.GA3
    )
...
def test_directly_constructed_ga12_ready_with_synthetic_ga11_history_is_denied() -> (
    None
):
    target = contract_at(
        GicaGate.GA12, GicaProgramState.READY_FOR_FOUNDER, history=(GicaGate.GA11,)
    ...
    with pytest.raises(
        ProgramTransitionError,
        match="legitimate_ga11_to_ga12_transition_proof_required",
    ):
        target.founder_promote(authorization=founder(), now=NOW)


def test_externally_anchored_reserved_founder_authorization_permits_complete_only_at_final_gate() -> (  # noqa: E501
    None
):
    ga12 = advance(
        contract_at(GicaGate.GA11),
    ...
    assert (
        ga12.founder_promote(authorization=founder(), now=NOW).state
        is GicaProgramState.COMPLETE
    )
