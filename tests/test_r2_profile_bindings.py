...
def test_ten_distinct_profiles_bind_same_frozen_kernel() -> None:
    validate_profiles()
    assert len(PROFILES) == 10
    assert {profile.kernel_interface_ref for profile in PROFILES.values()} == {
        KERNEL_INTERFACE_REF
    }
    assert len({profile.identity for profile in PROFILES.values()}) == 10
    assert len({profile.specialty for profile in PROFILES.values()}) == 10
...
def test_namespaces_are_ocs_local_and_isolated() -> None:
    ...
    assert len(state_namespaces) == 10
    assert len(memory_namespaces) == 10
...
def test_specialty_boundaries_match_noesis_manifest() -> None:
...
def test_self_assurance_denied_for_relevant_profiles() -> None:
    for ocs_id in ("NÓESIS", "DÉDALA", "SÝNESIS", "ÍRIS", "SOFIA", "ÁGORA"):
