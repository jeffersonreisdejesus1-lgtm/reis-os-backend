...
from app.cognitive_physiology.contracts import Candidate, EpistemicGrade, MemoryLevel, OperationalCommitContext
from app.cognitive_physiology.engine import ExpectedOutcome, ObservedOutcome, UniversalCognitiveEngine
...
def candidate(candidate_id: str, *, salience: float, risk: float = 0.1) -> Candidate:
    return Candidate(
...
def test_cycle_competes_broadcasts_observes_and_records_episode_without_world_commit() -> None:
    ...
    result = engine.run_cycle(
        ...
        candidates=(candidate("low", salience=0.2), candidate("high", salience=0.9)),
    ...
    assert engine.memory.get(MemoryLevel.M2, result.episodic_record.record_id) == result.episodic_record


def test_material_action_requires_explicit_operational_commit() -> None:
    engine = UniversalCognitiveEngine(runtime())
    with pytest.raises(PermissionError, match="material_action_requires_explicit_operational_commit"):
        engine.run_cycle(
...
def test_full_operational_commit_executes_effect_but_does_not_implicitly_commit_world() -> None:
    rt = runtime()
    engine = UniversalCognitiveEngine(rt)
    effects: list[str] = []
    result = engine.run_cycle(
...
def test_prediction_residual_updates_nm_without_granting_authority() -> None:
    ...
    result = engine.run_cycle(
        ...
        expected=ExpectedOutcome(expected_observation={"value": 1}, expected_effect={"done": True}),
        observe=lambda: ObservedOutcome(observation={"value": 2}, effect={"done": False}),
        generation=0,
...
def test_m2_to_m3_requires_qualification_and_persistence_authority() -> None:
    ...
    with pytest.raises(PermissionError, match="m3_persistence_authority_required"):
        engine.memory.consolidate(
            source_record_id=source_id,
            target_record_id="semantic-1",
            qualified=True,
