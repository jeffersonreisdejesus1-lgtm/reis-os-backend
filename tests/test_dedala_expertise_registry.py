...
def test_dedala_manifest_is_optional_specialty_capability() -> None:
    ...
    assert DEDALA_EXPERTISE_MANIFEST.capability_status == "OPTIONAL_SPECIALTY_CAPABILITY"
...
def test_problem_selects_distributed_state_and_messaging_lenses() -> None:
    plan = build_retrieval_plan(
        "Hazel Kernel distributed state consistency causality recovery message retry delivery"
    )
...
def test_refactoring_and_boundary_problem_selects_fowler_and_newman_lenses() -> None:
    plan = build_retrieval_plan(
        "refactor legacy architecture boundaries coupling migration without a monolith"
    )
...
def test_expertise_cannot_create_authority() -> None:
    ...
    with pytest.raises(ValueError, match="expertise_must_not_create_authority"):
        validate_manifest(invalid)
