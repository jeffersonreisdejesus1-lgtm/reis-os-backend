...
from app.noesis_r7 import (
    CommunicationEnvelope, GovernorContract, GovernorFunction, GovernorLease,
    GovernanceCommand, GovernanceTask, R1R6IntegrationContract,
    R7GovernanceRuntime, R7InvariantError,
)
...
def governor(governor_id: str = "governor:state", *, owned=("current_phase", "blockers"), function=GovernorFunction.STATE):
    return GovernorContract(governor_id=governor_id, function=function,
        owned_state_keys=tuple(owned), readable_state_keys=tuple(owned) + ("mission_id",),
        allowed_commands=("SET_STATE", "RECONCILE"), authority_ceiling_ref=AUTH)


def lease(governor_id="governor:state", *, lease_id="lease:r7:1", generation=1):
    return GovernorLease(lease_id=lease_id, governor_id=governor_id, mission_id=MISSION,
        authority_ref=AUTH, authority_source_ref=SOURCE, scope=("state:noesis:r7",),
        generation=generation, issued_at=100.0, not_before=100.0, expires_at=500.0,
        max_uses=20)


def command(*, command_id="cmd:1", governor_id="governor:state", lease_id="lease:r7:1",
            generation=1, write_set=None, idempotency_key="idem:1", expected_state_version=0,
            command_type="SET_STATE", material_effect_requested=False):
    return GovernanceCommand(command_id=command_id, mission_id=MISSION, governor_id=governor_id,
        generation=generation, lease_id=lease_id, authority_ref=AUTH, command_type=command_type,
        write_set=write_set or {"current_phase": "IMPLEMENTATION"},
        idempotency_key=idempotency_key, expected_state_version=expected_state_version,
        scope=("state:noesis:r7",), material_effect_requested=material_effect_requested)


def runtime():
    rt = R7GovernanceRuntime(mission_id=MISSION, integration=R1R6IntegrationContract.canonical(),
...
def admit(rt, cmd, *, priority=1):
    rt.schedule(GovernanceTask(task_id=f"task:{cmd.command_id}", mission_id=MISSION,
        governor_id=cmd.governor_id, command_type=cmd.command_type, priority=priority,
        created_seq=rt._task_seq + 1, command_id=cmd.command_id))
...
def test_exact_r1_r6_binding_and_l0_fail_closed():
...
def test_cross_owner_write_and_material_effect_fail_closed():
    ...
    cross = command(command_id="cmd:cross", governor_id="governor:evidence", lease_id="lease:evidence",
        idempotency_key="idem:cross", write_set={"current_phase": "FORGED"})
...
def test_communication_never_transfers_authority():
    rt = runtime()
    rt.register_governor(governor("governor:communication", owned=("last_message_ref",), function=GovernorFunction.COMMUNICATION))
    rt.communicate(CommunicationEnvelope("msg:1", MISSION, "governor:state", "governor:communication",
    ...
    with pytest.raises(R7InvariantError, match="MUST_NOT_TRANSFER_AUTHORITY"):
        CommunicationEnvelope("msg:bad", MISSION, "governor:state", "governor:communication",
...
def test_generation_fencing_and_expiry_fail_closed():
    ...
    current = command(command_id="cmd:gen2", idempotency_key="idem:gen2", lease_id="lease:gen2", generation=2)
...
def test_owner_scoped_recovery_preserves_other_governor_state_and_monotonic_version():
    ...
    execute(rt, command(command_id="cmd:block", idempotency_key="idem:block", expected_state_version=1,
        write_set={"blockers": ["b1"]}), now=112.0)
    execute(rt, command(command_id="cmd:evidence", governor_id="governor:evidence", lease_id="lease:evidence",
        idempotency_key="idem:evidence", expected_state_version=2, write_set={"evidence_refs": ["e1"]}), now=113.0)
...
def test_rollback_counts_only_real_deltas_and_invalidates_later_idempotency():
    ...
    later = command(command_id="cmd:later", idempotency_key="idem:later", expected_state_version=1,
