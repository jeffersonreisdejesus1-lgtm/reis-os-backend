...
def test_t01_t02_t03_real_write_readback_and_tests(tmp_path: Path) -> None:
    ...
    assert "probe_digest" in provenance
    assert "artifact_digest" in provenance
    assert len(provenance["probe_digest"]) == 64
    assert len(provenance["artifact_digest"]) == 64
...
def test_t10_restart_new_store_object(tmp_path: Path) -> None:
    ...
    assert again.readback.reconciliation is Reconciliation.MATCH
...
def test_t18_different_intent_hash_conflict(tmp_path: Path) -> None:
    plane = _plane(tmp_path)
    plane.execute(_req())
    conflict = plane.execute(_req(authorized_intent_hash="other"))
    assert conflict.failure == "effect_identity_conflict"
    assert conflict.material is False
...
def test_t19_different_object_conflict(tmp_path: Path) -> None:
    plane = _plane(tmp_path)
    plane.execute(_req())
    conflict = plane.execute(_req(bound_object="object://other"))
    assert conflict.failure == "effect_identity_conflict"
    assert conflict.material is False
...
def test_t20_t21_expected_version_enforced(tmp_path: Path) -> None:
    plane = _plane(tmp_path)
    first = plane.execute(_req())
    artifact = Path(first.material_artifact_ref)
    ...
    assert artifact.read_text(encoding="utf-8") == "R$ 25,90"
    ok = plane.execute(
        _req(
            effect_id="eff-3",
            logical_operation_id="op-3",
            expected_object_version=content_hash,
            payload="next",
        )
    )
