...
from app.cognitive_physiology.local_profiles import LOCAL_PROFILES, validate_local_profiles
...
def test_eleven_profiles_are_distinct_and_noesis_is_reference_only() -> None:
    ...
    assert LOCAL_PROFILES["TÊMIS"].institutional_id == "REISOS::INST::ANDROID_PLAY::001"
...
def test_operational_commit_fails_closed_when_any_predicate_missing(context: OperationalCommitContext) -> None:
    rt = runtime()
    effects: list[str] = []
...
def test_institutional_commit_requires_authority_and_r6() -> None:
    rt = runtime()
    with pytest.raises(PermissionError, match="institutional_commit_authority_required"):
    ...
    with pytest.raises(PermissionError, match="institutional_commit_r6_validation_required"):
        rt.institutional_commit(
            transition_id="t-2",
            expected_generation=0,
            changes={"status": "x"},
...
def test_handoff_denies_unresolved_authority() -> None:
    with pytest.raises(PermissionError, match="authority_unresolved"):
        handoff(authority_resolution_status="UNRESOLVED").validate_for_target(target_generation=7)


def test_handoff_denies_unresolved_capability() -> None:
    with pytest.raises(PermissionError, match="capability_unresolved"):
        handoff(capability_resolution_status="UNRESOLVED").validate_for_target(target_generation=7)


def test_handoff_denies_stale_envelope() -> None:
    stale = handoff(expires_at=datetime.now(timezone.utc) - timedelta(seconds=1))
